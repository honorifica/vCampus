...
public class ClientTeacherFrame extends JFrame implements ActionListener{
	 ...
	 * 
	 *  ClientTeacherFrame
	 *  ClientTeacherFrame构造函数
	 * @param number 登录老师的姓名
	 * @param socket1 用来通信
	 ...
	 *  2021-07-28 02:32:40
	 */
	public ClientTeacherFrame(String number,Socket socket1) throws ClassNotFoundException,SQLException,IOException
	{
		this.socket=socket1;
		setUndecorated( true);//去边框
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		//setBounds(100, 100, 958, 525);
		...
		//JPanel Clock = new JPanel();
		Clock.setBounds(598, 35, 223, 201);
		...
		Function.setBounds(10, 21, 173, 488);
		...
		JButton Stu_infoButton = new JButton("\u5B66\u7C4D\u7BA1\u7406");
		Stu_infoButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					ClientStudentManegerFrame csmf= new ClientStudentManegerFrame(number,socket);
				} catch(Exception e2) {
					e2.printStackTrace();
		...
		JButton Stu_courseButton = new JButton("\u8BFE\u7A0B\u7BA1\u7406");
		Stu_courseButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					ClientCourseFrame cscf= new ClientCourseFrame(number, socket);
				} catch(Exception e2) {
					e2.printStackTrace();
				}
			}
		...
		back_icon1.setBounds(240, 100, 211, 149);
		...
		back_icon2.setBounds(187, 100, 274, 245);
		contentPane.add(back_icon2);

		JPanel CalendarPart = new JPanel();
		CalendarPart.setBounds(505, 242, 425, 267);
